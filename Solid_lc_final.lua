-- credit to gelatek | Fixed by AndrFix#0880 B)
...
local script = game:GetObjects("rbxassetid://6661734630")[1]:FindFirstChildOfClass("Script")
...
local BruhRemotes = Instance.new("Folder",game.Players.LocalPlayer.Character)
...
local Chatted = Player.Chatted:Connect(function(Message)
	...
	if Message == Message_ then
		if HideIdentity then
			Event:FireServer("Chat",{Starter = Name, Text = Message})
		else
			Event:FireServer("Chat",{Starter = Name, Text = Message})
		end
	end
	...
	local MainLoop = game:GetService("RunService").Heartbeat:Connect(function(s)
		...
		for i,v in pairs(CharacterProps) do
			if not Character[i] or not obamagaming.Refit(Character[i],workspace) then
				...
				Character[i].CanCollide = false
				Character[i].CanQuery = false
				Character[i].CanTouch = false
...
Character[i].Transparency = 1
		...
		Event:FireServer("SetCharacter",{Character = game.Players.LocalPlayer.Character,MusicHolder = MusicHolder,Music = Music,WalkSpeed = Movement.WalkSpeed})
	end)
...
game:GetService("RunService"):BindToRenderStep(RandomString(),199,function()
	...
	if (Movement.PotentialCFrame.X ~= OldCFrame.X or Movement.PotentialCFrame.Z ~= OldCFrame.Z) then
		...
		Movement.CFrame = CFrame.new(Movement.CFrame.Position,Movement.PotentialCFrame.Position)*CFrame.new(0,0,(1/60)*-Movement.WalkSpeed)
	else
...
coroutine.resume(coroutine.create(function()
	while true do
		...
		tecks2.TextColor3 = cColor.Value
	end
end))
...
function Notify(StarterText,Text)
	...
	coroutine.resume(coroutine.create(function()
		...
		coroutine.resume(coroutine.create(function()
			while true do
				wait()
				NotifText.TextColor3 = cColor.Value
			end
		end))
...
game.StarterGui:SetCore("SendNotification",  {
 Title = "fixed by AndrFix#0880";
 Text = "Solid lc loaded";
 Icon = "";
 Duration = 5;
